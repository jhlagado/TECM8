...
| ---: | --- | ---: | ---: | ---: | ---: |
| 0 | Shell, launcher, registry | `995` | `995` | `83E3h` | `15389` |
...
Expansion occupied bytes: `2666`

Expansion high-water span total: `2666`

Latest TEC-FS geometry-line milestone delta:
...
| Bank 0 service registry | `83BAh` | Private service ID to bank/address/target-`A` table. |
| Bank 0 shell entry | `809Ch` | Private descriptor and VDU home-screen path for `SHL_ENTRY`. |
| Bank 0 shell command boundary | `80EBh` | Private one-command dispatcher reached through `SHL_RUN_COMMAND`. |
| Bank 0 info marker | `83B5h` | Private marker, not a fixed ABI location. |
