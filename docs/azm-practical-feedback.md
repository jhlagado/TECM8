...
- Current helper extraction: adding `src/tecm8-record.asm` moves masked
  ...
  by shell/editor-navigation wrappers, the shell's ASCII-space scanner, and the
  shell's local-name path scanner. `npm run z80:size` reports 15,090 bytes, a
  145-byte reduction from the
