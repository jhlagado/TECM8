...
test('monitor register contract audit is documented and wired to package scripts', () => {
  ...
  assert.match(report, /Strict contract diagnostics: `281`/);
  assert.match(report, /roms\/tec1g\/tecm8\/monitor\/monitor\.asm \| 186/);
  ...
  assert.match(report, /Consider asking AZM for a quarantine mechanism/);
