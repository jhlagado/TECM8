...
async function main(): Promise<void> {
  ...
  const trace = readTrace(runtime, traceBase, 26);
  ...
  assertEqual(trace[14], 0x60, 'VDU status-line restored cursor low byte');
  ...
  assertEqual(trace[22], 0x60, 'VDU cursor low preserved after unknown selectors');
  assertEqual(trace[23], 0x01, 'VDU cursor high preserved after unknown selectors');
  assertEqual(trace[24], 0x81, 'VDU counted string return value');
  assertEqual(trace[25], 0x81, 'VDU counted string zero terminator return value');
  ...
  assertEqual(tms9918.vram[0x0130] ?? 0, 0x41, 'VDU counted string first character write');
  assertEqual(tms9918.vram[0x0131] ?? 0, 0x42, 'VDU counted string second character write');
  assertEqual(tms9918.vram[0x0132] ?? 0, 0x43, 'VDU counted string third character write');
  assertEqual(tms9918.vram[0x0133] ?? 0, 0x20, 'VDU counted string stops at count');
  assertEqual(tms9918.vram[0x0140] ?? 0, 0x58, 'VDU counted string before zero first character write');
  assertEqual(tms9918.vram[0x0141] ?? 0, 0x59, 'VDU counted string before zero second character write');
  assertEqual(tms9918.vram[0x0142] ?? 0, 0x20, 'VDU counted string stops at zero before count');
  ...
  assertEqual(tmsParams[4], 0x60, 'VDU cursor low after newline');
