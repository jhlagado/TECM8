...
test('TMS9918 bank proof covers VDU cursor and put-char behavior', () => {
  ...
  assert.match(bank1, /cp VDU_SVC_SCROLL_UP\+1/);
  assert.match(bank1, /Tecm8VduServiceTable:[\s\S]*jp\s+vduSetCursorImpl[\s\S]*jp\s+vduPutCharImpl[\s\S]*jp\s+vduPutStringImpl[\s\S]*jp\s+vduNewlineImpl[\s\S]*jp\s+vduSetRowColImpl[\s\S]*jp\s+vduScrollUpImpl/);
