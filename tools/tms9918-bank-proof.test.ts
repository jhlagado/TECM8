...
test('TMS9918 bank proof covers VDU cursor and put-char behavior', () => {
  ...
  assert.match(bank1, /cp TMS_SVC_FILL_VRAM\+1/);
  assert.match(bank1, /Tecm8TmsServiceTable:[\s\S]*jp\s+tmsInitImpl[\s\S]*jp\s+tmsSetRegisterImpl[\s\S]*jp\s+tmsWriteVramImpl[\s\S]*jp\s+tmsFillVramImpl/);
  ...
  assert.match(runner, /VDU clear final name-table byte/);
