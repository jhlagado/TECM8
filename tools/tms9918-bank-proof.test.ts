...
test('TMS9918 bank proof covers VDU cursor and put-char behavior', () => {
  ...
  assert.match(bank1, /cp TECM8_VDU_SVC_NEWLINE\+1/);
  ...
  assert.match(bank1, /vduSetCursorImpl:[\s\S]*ld hl,\(TECM8_TMS_PARAM_ADDR_LO\)[\s\S]*ld \(TECM8_TMS_PARAM_CURSOR_LO\),hl[\s\S]*ld a,0x81[\s\S]*ret/);
  assert.match(bank1, /vduPutCharImpl:[\s\S]*TECM8_TMS_PARAM_CURSOR_LO[\s\S]*call tmsWriteVram[\s\S]*inc hl[\s\S]*ld a,0x81[\s\S]*ret/);
  assert.match(bank1, /vduPutStringImpl:[\s\S]*ld hl,\(TECM8_TMS_PARAM_STRING_LO\)[\s\S]*call vduPutChar[\s\S]*jr vduPutStringNext[\s\S]*ld a,0x81[\s\S]*ret/);
  assert.match(bank1, /vduNewlineImpl:[\s\S]*and 0xE0[\s\S]*ld de,TECM8_VDU_TEXT_ROW_BYTES[\s\S]*add hl,de[\s\S]*ld a,0x81[\s\S]*ret/);
  assert.match(proof, /callBankService 0x01,TECM8_VDU_SERVICE_CALL,TECM8_VDU_SVC_SET_CURSOR/);
  assert.match(proof, /callBankService 0x01,TECM8_VDU_SERVICE_CALL,TECM8_VDU_SVC_PUT_CHAR/);
  assert.match(proof, /callBankService 0x01,TECM8_VDU_SERVICE_CALL,TECM8_VDU_SVC_PUT_STRING/);
  assert.match(proof, /callBankService 0x01,TECM8_VDU_SERVICE_CALL,TECM8_VDU_SVC_NEWLINE/);
