#!/usr/bin/env node
/**
 * Assemble and run the shared TECM8 string-helper proof in Debug80's Z80 runtime.
 */
...
async function compileProof(): Promise<{ bytes: Uint8Array; symbols: D8Symbol[] }> {
  ...
  const result = await compile(
    PROOF_SOURCE,
    {
      ...
      d8mInputs: {
        bin: 'build/tecm8-string-proof.bin',
      },
      registerCare: 'strict',
...
async function main(): Promise<void> {
  ...
  const copyCase = runtime.hardware.memory[caseAddr];

  if (result !== PROOF_PASS) {
    throw new Error(`tecm8 string proof failed: marker=0x${result.toString(16)} case=${copyCase}`);
  }
