...
test('banked service ABI doc covers bank 2 TEC-FS slots and parameters', () => {
  ...
  assert.match(doc, /caller-owned RAM\s+pointer to two adjacent catalogue slots/);
  assert.match(doc, /does not read SD\s+sectors, scan an arbitrary catalogue, or allocate a directory cursor/);
  assert.match(doc, /summarizes the selected slot, advances once, summarizes the next slot, restores\s+the original pointer/);
